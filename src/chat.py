...
def generate_response(
    openai_key, user_message: str, context: str, conversation_history: List[Dict], model: str = "gpt-3.5-turbo"
) -> str:
...
def evaluate_response_quality(question: str, answer: str, contexts: List[str]) -> Dict[str, float | str]:
...
def display_evaluation_metrics(scores: Dict[str, float]):
    ...
    for metric_name, score in scores.items():
        if isinstance(score, (int, float)):
            ...
            st.sidebar.metric(label=metric_name.replace("_", " ").title(), value=f"{score:.3f}", delta=None)
...
def main():
    ...
    with st.sidebar:
        ...
        openai_key = st.text_input(
            "OpenAI API Key", type="password", value=os.getenv("OPENAI_API_KEY", ""), help="Enter your OpenAI API key"
        )
            ...
            with st.spinner("Searching documents and generating response..."):
                ...
                if enable_evaluation and RAGAS_AVAILABLE:
                    with st.spinner("Evaluating response quality..."):
                        evaluation_scores = evaluate_response_quality(prompt, response, contexts_list)
